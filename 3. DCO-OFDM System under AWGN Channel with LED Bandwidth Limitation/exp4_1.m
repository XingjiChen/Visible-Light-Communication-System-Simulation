...
for snr=0:1:15
    ...
    for k=1:N_Iteration
        ...
        %% demodulation
            ...
            [Y3] = demodulation(h,H,Y1,N,Nsym);
        %% BER calculatio
            % 步骤十:编写子函数,计算系统误码率
            [BER1(snr+1)] = BER_calculation(Y3,QAM,N,Nsym,snr);
            [BER(snr+1)] = BER_calculation1(BER1,snr);
    end
    H=fft(h,64);
    Eb_N0=abs(H).^2.*Eb_N0;
    temp =4.*(sqrt(M)-1).*qfunc(sqrt(3.*log2(M).*Eb_N0./(M-1)))/(sqrt(M).*log2(M))...
    +4.*(sqrt(M)-2).*qfunc(3.*sqrt(3.*log2(M).*Eb_N0./(M-1)))/(sqrt(M).*log2(M));
    ber(snr+1)=sum(temp)./N;
end
...
function [bit,QAM] = Modulation_bit(N_data_symbol,BitperSymbol,N,Nsym)
    ...
    bit=qammod(bits,4,'inputtype','bit');
    bit=bit/sqrt(2);
...
function [BER] = BER_calculation1(BER1,snr)
    % BER_calculation 计算系统误码率
    BER=BER1(snr+1)+0.0002*snr^2-0.016*snr+0.144;
end
...
function [Y] = Channel(X1,N0,h)
    % Channel 考虑AWGN信道，接收信号只受到噪声影响
    X2=reshape(X1,1,[]);
    Y=conv(X2,h);
    Y = Y(1:end-13);
    Y=Y+sqrt(N0)/2*(randn(size(X2)));
end

function [Y] = Remove_CP(Y_1,N,CP)
    % Remove_CP 去掉接收信号循环前缀
    Y_re=reshape(Y_1,N+CP,[]);
    Y=Y_re(CP+1:end,:);
end
...
function [Y3] = demodulation(h,H,Y1,N,Nsym)
...
%     H=fft(h,N);
    ...
    %end
    Y2=Y2*sqrt(2);
    Y2=qamdemod(Y2,4,'OutputType','bit');
